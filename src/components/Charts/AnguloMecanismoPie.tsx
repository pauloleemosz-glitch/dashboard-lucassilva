...
function PieTooltip({ active, payload, total, mode }: any) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="capitalize" style={{ color: p.payload.fill }}>{p.name}</span>
      </div>
      <div className="text-foreground tabular-nums">
        {isLeads ? `${formatNumber(p.value)} leads` : `${formatNumber(p.value)} vendas`}
      </div>
      {!isLeads && typeof p.payload.revenue === "number" && p.payload.revenue > 0 && (
...
export function AnguloMecanismoPie({
  title,
  data,
  mode = "vendas",
  delay = 0,
      ...
      {filtered.length === 0 ? (
        ...
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            <Tooltip content={<PieTooltip total={total} mode={mode} />} />
            <Legend
              ...
              formatter={(v: string, entry: any) => {
                ...
                const suffix = !isLeads && rev > 0
                  ? formatBRL(rev)
                  : `${formatNumber(val)} ${isLeads ? "leads" : "vendas"}`;
                return (
                  <span style={{ color: "hsl(var(--muted-foreground))" }} className="capitalize">
                    {v}
